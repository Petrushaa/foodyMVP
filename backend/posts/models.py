...
class Taxon(models.Model):
    """
    Категория по одной из осей. Все оси лежат в одной таблице, потому что
    осей со временем станет больше, а фильтры и админка так пишутся один раз.

    Осей две: кухня и вид. «Вид» — это бывшие формат, форма и дополнительно,
    слитые в одну ось: делить «фастфуд», «бургеры» и «веганское» по разным
    вкладкам значило бы заставлять человека угадывать, в какой из трёх лежит
    нужное. Одно блюдо спокойно бывает и фастфудом, и веганским сразу.
    """
    ...
    # Оси, где у позиции может быть только одно значение. Вид — сколько угодно:
    # фастфуд + веганское + острое одновременно это нормально.
    SINGLE_VALUE_KINDS = (KIND_CUISINE,)
