...
class MenuItemViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _apply_city(self, queryset):
        ...
        Сужаем только выдачу списка. Страница позиции и её посты открываются
        по прямой ссылке из чужого города — упереться в «не найдено», перейдя
        по присланной ссылке, было бы поломкой. Конкретное заведение — по той же
        причине. Гость и тот, кто не указал город, видят всё.
        """
        if self.action != 'list' or self.request.query_params.get('restaurant'):
            return queryset
